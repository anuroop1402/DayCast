...
nonisolated struct SearchCities: SearchCitiesUseCase {
    ...
    func callAsFunction(query: String) async throws -> [City] {
        ...
        // failure for it. The floor stops the first keystroke of every search from becoming
        // a request; debouncing in the ViewModel reduces *how often* we ask, this decides
        // *whether* to.
        guard CitySearchPolicy.isSearchable(trimmed) else { return [] }
