...
| Whose "today" is Today — the user's, or the city's? | **The city's own calendar day.** | The forecast is *about somewhere else*, so the days being ranked are that city's days — "Tuesday" means Chamonix's Tuesday. Open-Meteo returns the city's local days, and for a few hours each day they disagree with the device's. **Reversed after review** — the original choice was the device's calendar, on the grounds that it matches the calendar the user is holding. That reasoning only recorded the milder failure (a user *ahead* of the city sees a "Today" row that is the city's tomorrow) and missed the worse one: a user *behind* the city — India at 22:00 IST opening Shanghai — sees China's **current** conditions headed **"Tomorrow"**, a present-tense forecast sold as the future, with nothing on screen to contradict it. The remaining cost is the mirror case and is milder: for those hours a device on the 6th sees a row reading "Today, 7 Sep", and `shortDate` puts the calendar date beside every label. The forecast screen also carries a caption under the city name — *"It's 6:08 AM on Monday, 7 September in Queenstown"* — shown only when the two clocks differ, so the frame is stated rather than left to be inferred from a date. Implemented in `DayLabel`, where `cityTimeZone` is a required parameter with **no default** — a default of `.current` is the hidden environment read the type exists to prevent, and it would compile silently. |
